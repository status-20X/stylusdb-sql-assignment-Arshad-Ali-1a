...
// const student = require("../../student.csv")

test("Read CSV File", async () => {
  const data = await readCSV("./student.csv");
  expect(data.length).toBeGreaterThan(0);
  expect(data.length).toBe(4); // Update to 4
  ...
  expect(parsed).toEqual({
    ...
    whereClauses: [
      ...
      },
      {
        field: "name",
        operator: "=",
        value: "John", // Enclose 'John' in single quotes
      },
    ],
    joinCondition: null,
    joinTable: null,
...
test("Execute SQL Query with Greater Than", async () => {
  ...
  expect(result.length).toEqual(3); // Updated expectation to match the actual result
  expect(result[0]).toHaveProperty("id");
});

test("Execute SQL Query with Not Equal to", async () => {
  const queryWithNE = "SELECT name FROM student WHERE age != 25";
  const result = await executeSELECTQuery(queryWithNE);
  expect(result.length).toEqual(3); // Adjusted expectation to match the actual result
  expect(result[0]).toHaveProperty("name");
});
